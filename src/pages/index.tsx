...
import { getNameAndRegion } from "src/services/SearchCountries";
...
const Home: FC<Props> = ({ initialCountries }) => {
  ...
  const searchCountries = async () => {
    try {
      const countries = await getNameAndRegion(
        search.query,
        search.region
      );
  ...
  return (
    <main>
      <Head>
        <title>
          RESTCountries API - Frontend Mentor Challenge
        </title>
        <meta
          name="description"
          content="RESTCountries API Frontend Mentor Challenge by Djalma Neto"
        />
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const initialCountries = await getNameAndRegion("us");
  return {
    props: {
      initialCountries,
    },
